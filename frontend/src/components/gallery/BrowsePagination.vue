...
<template>
  <nav v-if="showBar" class="pagination-bar">
    <div class="page-size-controls">
      <span class="page-size-label">{{ t('page.perPage') }}</span>
      <button
    ...
    <template v-if="gallery.totalPages > 1">
      ...
      <button
        ...
        class="page-nav-btn"
        :title="t('page.prevTitle')"
        :disabled="gallery.page <= 1"
        @click="emit('changePage', gallery.page - 1)"
      >
        {{ t('page.prev') }}
      </button>
      <span class="page-info-text">{{ t('page.curPage', { cur: gallery.page, total: gallery.totalPages }) }}</span>
      <button
        type="button"
      ...
      <div class="page-list" :aria-label="t('page.numsAria')">
      ...
      <span class="page-info-text">{{ t('page.count', { n: gallery.total }) }}</span>
